...
class cloud_driver;
  ...
  begin
    ...
    fp = $fopen("d:/aidata/cloud_sv_simv.txt","w");
    doReading = 1;
    hasSaved = 0;
    printEndInst = 0;
    outSize = 0;
  ...
  begin
      ...
      if(ports.output_enable) begin
          ...
          if(outSize == 86190) begin
            $fclose(fin);
            fin = $fopen("d:/aidata/cloud_sv_input0.txt","r");
            doReading = 1;
          end
          else if(outSize == 86190 + 344760) begin
              iter = iter + 1;
              $fclose(fout);
              $fclose(fout2);
              $fclose(fp);
              ...
              fout2 = $fopen("d:/aidata/cloud_sv_outputx.txt","r");
              if((iter & 1) == 1)
                    fp = $fopen("d:/aidata/cloud_sv_simv1.txt","w");
              else
                    fp = $fopen("d:/aidata/cloud_sv_simv0.txt","w");
              hasSaved = 0;
              printEndInst = 0;
              outSize = 0;
          end
