...
class cloud_driver;
  ...
  begin
    ...
    fp = $fopen("d:/aidata/cloud_sv_sim.txt","w");
    doReading = 1;
    hasSaved = 0;
    printEndInst = 0;
    outSize = 0;
  ...
  begin
      ...
      if(ports.output_enable) begin
          ...
          for(int i = 0; i <  ports.output_len; i++) begin
            ...
            data8 = data;
            if(unsigned'(data8) != unsigned'(ports.output_data[i])) begin
                equal = 0;
                $fwrite(fp,"Error : Data differ addr %d (%d) i = %d, %d vs %d\n"
                , ports.output_addr, ports.output_len
            , i, data8, unsigned'(ports.output_data[i]));
            end
          end
          if(equal)
  ...
  begin
      ...
      for(int i = 0; i <  len_save; i++) begin
          if(unsigned'(data_saved[i]) != unsigned'(cloud_tb.cloud.outbuf[i])) begin
                equal = 0;
  ...
  begin
      ...
      if(cloud_tb.cloud.wen_data1) begin
          ...
          for(int i = 0; i <  len; i++) begin
            ...
            if(unsigned'(data8) != unsigned'(cloud_tb.cloud.outbuf[i])) begin
                equal = 0;
                $fwrite(fp,"Error : MemData differ addr %d (%d) i = %d, %d vs %d , %d\n"
                , cloud_tb.cloud.write_addr_data1, cloud_tb.cloud.write_len_data1
            , i, unsigned'(data8), unsigned'(cloud_tb.cloud.outbuf[i]), num);
            end
          end
          if(equal)
...
/*
      $fwrite(fp,"HCB0 hcbctrl, resetAssmbl %d, newconvinst %d, start_pre %d, kcur %d, kcurlast %d\n"
                , cloud_tb.cloud.hcb[0].h.hcbCtrl.resetAssemble, cloud_tb.cloud.hcb[0].h.hcbCtrl.new_conv_inst
                , cloud_tb.cloud.hcb[0].h.hcbCtrl.startPrefetching, cloud_tb.cloud.hcb[0].h.hcbCtrl.kCur
                , cloud_tb.cloud.hcb[0].h.hcbCtrl.kCurLast);
      $fwrite(fp,"HCB0 aib, doAssemble %d, inputs_wen %d, progfull %d, addr_next %d, curVol %d, isLoadingNF %d, isRF %d, nxtWP %d\n"
            , cloud_tb.cloud.hcb[0].h.assembleInputBuffer.doAssemble, cloud_tb.cloud.hcb[0].h.assembleInputBuffer.inputs_wen
            , cloud_tb.cloud.hcb[0].h.assembleInputBuffer.inputs_buffer_progfull, cloud_tb.cloud.hcb[0].h.assembleInputBuffer.addrNextAssemble_next
            , cloud_tb.cloud.hcb[0].h.assembleInputBuffer.curVolumeSize, cloud_tb.cloud.hcb[0].h.assembleInputBuffer.isLoadingNextFeatures
        , cloud_tb.cloud.hcb[0].h.assembleInputBuffer.isReadingFeatures, cloud_tb.cloud.hcb[0].h.assembleInputBuffer.nextWritePos);
*/
      ...
      if(WRITE_BUS) begin
          ...
          $fwrite(fp,"HCB0 hcbctrl, input_fifo_empty %d, new_conv_inst %d\n"
                ...
                , cloud_tb.cloud.hcb[0].h.weights_empty
          ...
          $fwrite(fp,"HCB1 out (%d %d), readfeature %d (%d,%d), readweight %d, froze %d, state %d, wempty %d, fempty (%d %d %d)\n"
                ...
                , cloud_tb.cloud.hcb[1].h.weights_empty
      ...
      $fwrite(fp,"HCB2 out (%d %d), readfeature %d (%d,%d), readweight %d, froze %d, state %d, wempty %d, fempty (%d %d %d)\n"
            ...
            , cloud_tb.cloud.hcb[2].h.weights_empty
      ...
      if(WRITE_BUS) begin
          $fwrite(fp,"HCB3 out (%d %d), readfeature %d (%d,%d), readweight %d, froze %d, state %d, wempty %d, fempty (%d %d %d)\n"
                ...
                , cloud_tb.cloud.hcb[3].h.weights_empty
